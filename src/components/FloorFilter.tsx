  ...
  return (
    <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-4 mb-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {floors.map(floor => {
              ...
              return (
                ...
                >
                  <span>{floor === 'ALL' ? '全校樓層總覽' : `${floor} 樓`}</span>
                  <span
        ...
        <div className="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs text-slate-700">
          <div className="flex items-center gap-1.5">
            <CheckCircle2 className="w-4 h-4 text-emerald-600" />
            <span>填報率: <strong className="text-slate-900">{reportedClasses}/{totalClasses}</strong> 班</span>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span>搬運完成: <strong className="text-slate-900">{completedClasses}/{totalClasses}</strong> 班</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            {totalDeskShortage > 0 ? (
              <span className="px-2 py-0.5 bg-rose-100 text-rose-800 font-bold rounded-md flex items-center gap-1">
                <AlertTriangle className="w-3 h-3 text-rose-600" />
                缺桌子 {totalDeskShortage} 張
              </span>
            ) : (
              <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 font-medium rounded-md">
                桌子無缺口
              </span>
            )}
